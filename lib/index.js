...
export default class Client {
   ...
   *
   * @param {number} type - ARTIST, ALBUM, TRACK
  ...
  allOfType (sectionId, type, options = {}) {
    ...
    return this.queryWithRange({...options, uri})
      .then((res) => this.parseType(type, res))
  }

  metadata (id, type) {
    const uri = `/library/metadata/${id}`
    return this.api.query({uri})
      .then((res) => this.parseType(type, res))
  }

  metadataChildren (id, type, options) {
    const uri = `/library/metadata/${id}/children`
    return this.api.query({uri, ...options})
      .then((res) => this.parseType(type, res))
  }

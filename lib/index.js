...
export default class Client {
  ...
  sectionItems (sectionId, type, options = {}) {
  ...
  albums (sectionId, options) {
    return this.sectionItems(sectionId, ALBUM, options)
  }
  ...
  artists (sectionId, options = {}) {
    return this.sectionItems(sectionId, ARTIST, options)
  }
  ...
  playlists (options = {}) {
    ...
    const uri = `/playlists/all?${params}`
    return this.queryWithRange({...options, uri})
      .then((res) => parsePlaylistContainer(res))
  }

  normalizedPlaylists (options) {
    return this.playlists(options)
      .then((data) => normalize(data, playlistContainerSchema))
  }

  playlist (id) {
    return this.api.query(`/playlists/${id}`)
      .then((res) => parsePlaylistContainer(res))
  ...
  playlistTracks (id, options) {
    const uri = `/playlists/${id}/items`
    return this.queryWithRange({...options, uri})
      .then((res) => parsePlaylist(res))
  }

  normalizedPlaylistTracks (id, options) {
    return this.playlistTracks(id, options)
      .then((data) => normalize(data, playlistSchema))
  }
  ...
  addToPlaylist (playlistId, uri) {
    ...
    return this.api.putQuery(`/playlists/${playlistId}/items?${params}`)
  }
  ...
  searchAll (query, limit = 3) {
    ...
    return this.api.query(`/hubs/search?${params}`)

...
export default class Library {
  ...
  createSmartPlaylist (title, uri) {
    return this.fetchJSON('/playlists', {
      method: 'POST',
      params: {
        type: 'audio',
        title,
  ...
  resizePhoto (params) {
    return this.api.getUrl('/photo/:/transcode', params)
  }
  ...
  trackSrc (track) {
    return this.api.getUrl(track.media[0].part[0].key)
  }
  ...
  playQueue (id) {
    return this.fetchJSON(`/playQueues/${id}`)
      .then((res) => parseType(QUEUE, res))
  }
